...
export default class RollConfigurationDialog extends DialogOP {
		...
		context.fields = [
			{
				field: new foundry.data.fields.StringField({
					label: game.i18n.localize("op.RollMode"),
					blank: false,
					required: true,
				}),
				name: "rollMode",
				value: this.message.rollMode ?? this.options.default?.rollMode ?? game.settings.get("core", "rollMode"),
				options: Object.entries(CONFIG.Dice.rollModes).map(([value, l]) => ({
					value,
					label: game.i18n.localize(`${game.release.generation < 13 ? l : l.label}`),
				})),
			},
		];
		return context;
	...
	#buildRolls(config, formData) {
		...
		this.#rolls =
			config.rolls?.map((config, index) => RollType.fromConfig(this._buildConfig(config, formData, index), this.config)) ??
			[];
	}
