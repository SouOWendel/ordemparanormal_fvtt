 ...
 * @returns {typeof BaseApplicationOP}
 * @mixin
 */
export default function ApplicationV2Mixin(Base) {
	/** */
	class BaseApplicationOP extends HandlebarsApplicationMixin(Base) {
		/** @override */
		static DEFAULT_OPTIONS = {
			actions: {
				toggleCollapsed: BaseApplicationOP.#toggleCollapsed,
			},
			classes: ["ordemparanormal"],
			window: {
				subtitle: "",
			},
		};
		...
		*_getHeaderControlContextEntries() {
		...
		async _prepareContext(options) {
			...
			// context.inputs = { ...foundry.applications.fields, ...op.applications.fields };
			return context;
		}
		...
		_onRender(context, options) {
			...
			this.element
				...
				.forEach((control) => control.addEventListener("click", ContextMenu5e.triggerEvent));
